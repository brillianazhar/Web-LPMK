<?php
	...
	if(isset($_SESSION['tempfbuser']))
	{
		...
		$que1=mysqli_query($konek,"select * from users where Email='$user' ");
		$rec=mysqli_fetch_array($que1);
		$userid=$rec[0];
		$que2=mysqli_query($konek,"select * from user_secret_quotes where user_id=$userid");
		$rec2=mysqli_fetch_array($que2);
		$q2=$rec2[3];
		$a2=$rec2[4];
		if($q2=="" && $a2=="")
		{
			$que3=mysqli_query($konek,"select * from user_secret_quotes where user_id=$userid");
...
<?php
	if(isset($_POST['Finish']))
	{
		...
		mysqli_query($konek,"update user_secret_quotes set Question2='$que2',Answer2='$ans2' where user_id=$userid");
		
		$que_user_data=mysqli_query($konek,"select * from users where Email='$user';");
		$user_data=mysqli_fetch_array($que_user_data);
		$userid=$user_data[0];
		$user_join_time=$user_data[6];
		mysqli_query($konek,"insert into user_post(user_id,post_txt,post_time,priority) values($userid,'Join Faceback','$user_join_time','Public');");
		mysqli_query($konek,"insert into user_status values($userid,'Online')");
		mysqli_query($konek,"insert into user_info(user_id) values($userid)");

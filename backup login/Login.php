...
{error_reporting(1);
	...
	$que1=mysqli_query($konek,"select * from users where Email='$user' and Password='$pass'");
	...
	if($count1>0)
	{
		...
		$que6=mysqli_query($konek,"select * from users where Email='$user'");
		$rec6=mysqli_fetch_array($que6);
		$userid=$rec6[0];

		$que2=mysqli_query($konek,"select * from user_profile_pic where user_id=$userid");
		...
		if($count2>0)
		{
			...
			if($count3>0)
			{
				$que4=mysqli_query($konek,"select * from user_secret_quotes where user_id=$userid");
				...
				else
				{
					...
					$userid=$rec1[0];
					mysqli_query($konek,"update user_status set status='Online' where user_id='$userid'");
	...
	{
		$que5=mysqli_query($konek,"select * from users where Email='$user'");
